...
# Let's go back to more extreme weights but not as extreme as original
# The key is to create some challenge but not make it impossible
...
# Try intermediate weights - between balanced and extreme
intermediate_weights = [
    # Moderately challenging
    [3.0, 0.4, 0.35, 0.2, 0.15, 0.1, 0.05],
    [4.0, 0.5, 0.4, 0.25, 0.2, 0.12, 0.06],
    [5.0, 0.6, 0.5, 0.3, 0.25, 0.15, 0.08],
    [6.0, 0.7, 0.6, 0.35, 0.3, 0.18, 0.1],
    [2.5, 0.35, 0.3, 0.18, 0.12, 0.08, 0.04],
    [3.5, 0.45, 0.38, 0.22, 0.18, 0.11, 0.055],
]

print("Testing intermediate weights for ~94% baseline accuracy:")
print("="*60)

results = []

for i, weights in enumerate(intermediate_weights):
...
# Find best candidate for ~94% accuracy
print("\n" + "="*60)
print("BEST CANDIDATES FOR ~94% BASELINE ACCURACY:")
print("="*60)

# Sort by how close to 94%
target = 0.94
...
for i in range(min(3, len(sorted_results))):
    ...
    print(f"\nCandidate {i+1}: Set {idx+1}")
    print(f"  Weights: {result['weights']}")
    print(f"  Accuracy: {result['accuracy']:.4f} (diff from 94%: {abs(result['accuracy'] - target)*100:.1f}%)")
    print(f"  MSE: {result['mse']:.4f}")
    print(f"  Balance score: {result['balance_score']:.3f}")
    print(f"  Extremes: <0.1: {result['near_0']}, >0.9: {result['near_1']}")
...
if sorted_results:
    ...
    print("="*60)
    print("Replace in synthetic_class_data.py:")
    ...
    print(f"\nExpected baseline accuracy: ~{best_result['accuracy']:.1%}")
    print(f"This should keep fairness regularization below baseline!")