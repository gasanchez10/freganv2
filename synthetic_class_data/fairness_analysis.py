...
def compute_models_once(X, y, treatment_col='T'):
    ...
    _global_mu0 = RandomForestClassifier(n_estimators=100, random_state=42).fit(_global_X_features[_global_T==0], y[_global_T==0]).predict_proba(_global_X_features)[:, 1]
    _global_mu1 = RandomForestClassifier(n_estimators=100, random_state=42).fit(_global_X_features[_global_T==1], y[_global_T==1]).predict_proba(_global_X_features)[:, 1]
    ...
    try:
        if model_counterfactuals:
            ...
            if "cate_predictions_counterfactuals" in case_dir:
                ...
                test_y_hat = pd.read_csv(f"{cate_dir}/test_y_hat_1.0.0_binary_cate.csv")
                test_x = pd.read_csv(f"{cate_dir}/test_x_1.0.0_binary_cate.csv")
                test_t = pd.read_csv(f"{cate_dir}/test_t_1.0.0_binary_cate.csv")
                
                # Take only test portion (last 200 rows) to match dimensions
                test_y_hat = test_y_hat.tail(len(test_x))
                
                # Use counterfactual predictions (columns 0 and 1 for control and treatment)
                full_y = (test_y_hat.iloc[:, 1].values > 0.5).astype(int)  # Convert to binary
                full_X = test_x.copy()
                full_T = test_t.copy()
            else:
                # For GANITE counterfactuals, read from ganite_alpha_5 subdirectory
                alpha_dir = f"{case_dir}/ganite_alpha_5"
                if not os.path.exists(alpha_dir):
                    return None, None, None, None
                    
                # Load counterfactual predictions
                test_y_hat = pd.read_csv(f"{alpha_dir}/test_y_hat_1.0.0_binary_ganite_alpha5.csv")
                test_x = pd.read_csv(f"{alpha_dir}/test_x_1.0.0_binary_ganite_alpha5.csv")
                test_t = pd.read_csv(f"{alpha_dir}/test_t_1.0.0_binary_ganite_alpha5.csv")
            ...
            test_x = pd.read_csv(f"{case_dir}/test_x_1.0.0_binary2025.csv")
            test_t = pd.read_csv(f"{case_dir}/test_t_1.0.0_binary2025.csv")
            
            # Filter predictions to test set only (last 200 rows)
...
def create_fairness_visualizations(results_df):
    ...
    plt.xlabel('Alpha Value')
    plt.ylabel('Average Treatment Effect (ATE)')
    plt.title('ATE vs Alpha - All Dataset Types')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.xlabel('Alpha Value')
    plt.ylabel('Conditional Average Treatment Effect (CATE)')
    plt.title('CATE vs Alpha - All Dataset Types')
    plt.legend()
    plt.grid(True, alpha=0.3)
