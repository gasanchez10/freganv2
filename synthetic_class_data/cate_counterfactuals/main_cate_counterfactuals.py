# CATE Counterfactuals Analysis for Synthetic Class Data (Binary Classification)
...
print("=== CATE COUNTERFACTUALS ANALYSIS FOR SYNTHETIC CLASS DATA ===")

# Load data
print("Loading synthetic class data...")
train_x = pd.read_csv(os.path.join(data_dir, f'train_x_{version}_binary2025.csv'))
train_t = pd.read_csv(os.path.join(data_dir, f'train_t_{version}_binary2025.csv')).values.flatten()
train_y = pd.read_csv(os.path.join(data_dir, f'train_y_{version}_binary2025.csv')).values.flatten()

test_x = pd.read_csv(os.path.join(data_dir, f'test_x_{version}_binary2025.csv'))
test_t = pd.read_csv(os.path.join(data_dir, f'test_t_{version}_binary2025.csv')).values.flatten()
test_y = pd.read_csv(os.path.join(data_dir, f'test_y_{version}_binary2025.csv')).values.flatten()

# Load true counterfactuals for comparison
train_cf_manual = pd.read_csv(os.path.join(data_dir, f'train_cf_manual_{version}_binary2025.csv')).values.flatten()
test_cf_manual = pd.read_csv(os.path.join(data_dir, f'test_cf_manual_{version}_binary2025.csv')).values.flatten()

# Prepare full dataset
full_x = np.vstack([train_x, test_x])
full_t = np.concatenate([train_t, test_t])
full_y = np.concatenate([train_y, test_y])
full_cf_manual = np.concatenate([train_cf_manual, test_cf_manual])
...
# Calculate metrics (convert to binary for comparison with true counterfactuals)
counterfactuals_binary = np.round(counterfactuals_probs).astype(int)
correlation_with_true = np.corrcoef(counterfactuals_binary, full_cf_manual)[0, 1]
mae = np.mean(np.abs(counterfactuals_binary - full_cf_manual))

print(f"  Correlation with true counterfactuals: {correlation_with_true:.4f}")
print(f"  MAE with true counterfactuals: {mae:.4f}")
print(f"  Mean CATE: {np.mean(cate_estimates):.4f}")
print(f"  Std CATE: {np.std(cate_estimates):.4f}")

# Create potential outcomes structure (same as continuous case)
train_potential_y = []
test_potential_y = []
...
# Save all data arrays (same structure as continuous case)
data_arrays = [
...
fig.suptitle('CATE Counterfactuals Analysis - Synthetic Class Data', fontsize=16, fontweight='bold')
...
# Plot 2: Correlation with true counterfactuals
ax2.scatter(full_cf_manual, counterfactuals_binary, alpha=0.6, color='blue')
ax2.plot([0, 1], [0, 1], 'r--', lw=2)
ax2.set_title(f'Predicted vs True Counterfactuals\nCorr: {correlation_with_true:.3f}')
ax2.set_xlabel('True Counterfactuals')
ax2.set_ylabel('Predicted Counterfactuals')
...
# Save correlation results (same as continuous case)
with open(os.path.join(results_dir, "cate_correlation_results.txt"), "w") as f:
    f.write("CATE Counterfactuals Results:\n")
    f.write(f"Correlation with Manual CF: {correlation_with_true:.4f}\n")
...
summary_df = pd.DataFrame({
    'correlation': [correlation_with_true],
    'mae': [mae],
