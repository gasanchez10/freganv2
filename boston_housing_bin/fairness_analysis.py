...
def compute_models_once(X, y, treatment_col='T'):
    ...
    # Compute outcome models with error handling
    mu0_model = RandomForestClassifier(n_estimators=100, random_state=42).fit(_global_X_features[_global_T==0], y[_global_T==0])
    mu0_proba = mu0_model.predict_proba(_global_X_features)
    _global_mu0 = mu0_proba[:, 1] if mu0_proba.shape[1] > 1 else mu0_proba[:, 0]
    
    mu1_model = RandomForestClassifier(n_estimators=100, random_state=42).fit(_global_X_features[_global_T==1], y[_global_T==1])
    mu1_proba = mu1_model.predict_proba(_global_X_features)
    _global_mu1 = mu1_proba[:, 1] if mu1_proba.shape[1] > 1 else mu1_proba[:, 0]
    ...
    try:
        if model_counterfactuals:
            ...
            if "cate_predictions_counterfactuals" in case_dir:
                ...
                # Take only test portion to match dimensions
                ...
                # For regular predictions, use predictions.csv
                ...
                test_x = pd.read_csv(f"{case_dir}/test_x_1.0.0_binary.csv")
                test_t = pd.read_csv(f"{case_dir}/test_t_1.0.0_binary.csv")
...
def analyze_fairness_results():
    ...
    # Find all prediction case directories (excluding GANITE)
    if model_counterfactuals:
...
def create_fairness_visualizations(results_df):
    ...
    plt.xlabel('Alpha Value')
    plt.ylabel('Average Treatment Effect (ATE)')
    plt.title('ATE vs Alpha - Boston Housing (CATE Only)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.xlabel('Alpha Value')
    plt.ylabel('Conditional Average Treatment Effect (CATE)')
    plt.title('CATE vs Alpha - Boston Housing (CATE Only)')
    plt.legend()
    plt.grid(True, alpha=0.3)
