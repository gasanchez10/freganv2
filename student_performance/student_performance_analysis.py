...
print(f"G3 range: {y_clean.min()} - {y_clean.max()}")
...
plt.subplot(1, 3, 3)
importance = pd.Series(rf_model.feature_importances_, index=X_clean.columns).sort_values(ascending=False)
importance.head(10).plot(kind='barh')
plt.xlabel('Importance')
plt.title('Top 10 Features')

plt.tight_layout()
plt.savefig('student_performance_results.png', dpi=300, bbox_inches='tight')

print(f"\nTop 5 important features:")
print(importance.head().to_string())
print("\nPlot saved as 'student_performance_results.png'")

# Doubly Robust ATE Estimation (following Matheus Facure's implementation)
def doubly_robust_ate(X, y, treatment_col='sex'):
    """
    Estimate Average Treatment Effect using Doubly Robust estimation.
    Implementation based on: https://matheusfacure.github.io/python-causality-handbook/12-Doubly-Robust-Estimation.html
    """
    from sklearn.ensemble import RandomForestClassifier, RandomForestRegressor
    
    # Create binary treatment from sex column (already binary after label encoding)
    ...
    # Step 3: Doubly Robust ATE estimation using the exact formula from the reference
    # ATE = E[T(Y-μ₁(X))/π(X) + μ₁(X)] - E[(1-T)(Y-μ₀(X))/(1-π(X)) + μ₀(X)]
    
    ate_dr = (np.mean(T*(y - mu1)/ps + mu1) - 
              np.mean((1-T)*(y - mu0)/(1-ps) + mu0))
    
    # Also calculate regression-only ATE for comparison
...
dr_results = doubly_robust_ate(X_clean, y_clean)
...
print(f"\nInterpretation: Gender difference in G3 scores is {dr_results['ate_doubly_robust']:.4f} points (on average)")

# Plot ATE analysis
plt.figure(figsize=(20, 5))

# Plot 1: G3 Score Distribution by Treatment
plt.subplot(1, 4, 1)
T = X_clean['sex'].astype(int)
plt.hist(y_clean[T==0], alpha=0.7, label='Female', bins=20, density=True)
plt.hist(y_clean[T==1], alpha=0.7, label='Male', bins=20, density=True)
plt.xlabel('G3 Score')
plt.ylabel('Density')
...
plt.subplot(1, 4, 3)
y_control = y_clean[T==0]
y_treated = y_clean[T==1]
plt.boxplot([y_control, y_treated], labels=['Female', 'Male'])
plt.ylabel('G3 Score')
...
plt.subplot(1, 4, 4)
# Show heterogeneity by failures
if 'failures' in X_clean.columns:
    key_var = 'failures'
    var_median = X_clean[key_var].median()
    high_var = X_clean[key_var] > var_median
    
    # Calculate ATE for each subgroup
    ate_high = np.mean(y_clean[T==1][high_var[T==1]]) - np.mean(y_clean[T==0][high_var[T==0]])
    ate_low = np.mean(y_clean[T==1][~high_var[T==1]]) - np.mean(y_clean[T==0][~high_var[T==0]])
...
plt.tight_layout()
plt.savefig('student_performance_ate_analysis.png', dpi=300, bbox_inches='tight')
print("\nATE analysis plots saved as 'student_performance_ate_analysis.png'")
...
def doubly_robust_cate(X, y, treatment_col='sex'):
    ...
    Returns individual-level treatment effects.
    """
    from sklearn.ensemble import RandomForestClassifier, RandomForestRegressor
...
cate_results = doubly_robust_cate(X_clean, y_clean)
...
for var in numeric_cols[:3]:  # Check first 3 numeric variables
    if var != 'sex' and var in X_clean.columns:
        var_values = X_clean[var]
...
if len(numeric_cols) > 1:
    ...
    plt.scatter(X_clean[key_var], cate_individual, alpha=0.6)
...
plt.tight_layout()
plt.savefig('student_performance_cate_analysis.png', dpi=300, bbox_inches='tight')
print("CATE analysis plots saved as 'student_performance_cate_analysis.png'")
...
# Use sex as treatment variable (already binary after label encoding)
treatment_col = 'sex'
...
print(f"Treatment variable: {treatment_col} (0=female, 1=male)")
print(f"Outcome variable: G3 (final grade)")
print(f"Features: {train_x.shape[1]} variables (excluding treatment)")

print("\n" + "="*50)
print("ANALYSIS SUMMARY")
print("="*50)
print(f"Dataset: Student Performance (n={len(X_clean)})")
print(f"Treatment: Gender (sex)")
print(f"Outcome: G3 scores (continuous)")
...
print("\nFiles generated:")
print("- student_performance_results.png (main analysis)")
print("- student_performance_ate_analysis.png (ATE plots)")
print("- student_performance_cate_analysis.png (CATE plots)")
