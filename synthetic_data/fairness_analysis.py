    ...
    try:
        if model_counterfactuals:
            ...
            if "cate_predictions_counterfactuals" in case_dir:
                ...
                # Take only test portion (last 300 rows) to match dimensions
            ...
            # Filter predictions to test set only (last 300 rows)
...
def analyze_fairness_results():
    ...
    for case_dir in case_dirs:
        ...
        # Extract alpha value from case name (e.g., ganite_α3_average_alpha_1.0)
    ...
    # Group by dataset type (manual, random, true, ganite_α3, cate)
    results_df['dataset_type'] = results_df['case_name'].str.extract(r'^(\w+)_average_alpha')
