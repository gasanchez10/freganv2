...
def compute_models_once(X, y, treatment_col='T', dataset_name="synthetic_class_data"):
    ...
    # Get optimal parameters for regression
    rf_params = get_optimal_params(dataset_name)
    
    # Compute propensity scores using classification (treatment is binary)
    from sklearn.ensemble import RandomForestClassifier
    ps_model = RandomForestClassifier(n_estimators=rf_params['n_estimators'], 
    ...
    # Compute outcome models using regression
    mu0_model = RandomForestRegressor(**rf_params, random_state=42)
    mu1_model = RandomForestRegressor(**rf_params, random_state=42)
...
def doubly_robust_ate_cate_regression(y, use_test_portion=False):
    """Calculate ATE and CATE using pre-computed global models for regression"""
    ...
    # Individual CATE estimates using doubly robust method for regression
...
def get_optimal_params(dataset_name):
    """Get optimal parameters for regression models"""
...
def analyze_fairness_across_alphas_regression(dataset_name):
    """Analyze fairness metrics across alpha values using regression approach"""
    ...
    if dataset_name == "synthetic_class_data":
        ...
        train_y = pd.read_csv(f"{data_path}train_y_1.0.0_binary2025.csv").iloc[:, 0].values.astype(float)
        ...
        test_y = pd.read_csv(f"{data_path}test_y_1.0.0_binary2025.csv").iloc[:, 0].values.astype(float)
    elif dataset_name == "german_credit":
        data_path = f"../{dataset_name}/data/"
        ...
        train_y = pd.read_csv(f"{data_path}train_y_1.0.0_continuous.csv").iloc[:, 0].values.astype(float)
        ...
        test_y = pd.read_csv(f"{data_path}test_y_1.0.0_continuous.csv").iloc[:, 0].values.astype(float)
    else:  # boston_housing_bin
        data_path = f"../{dataset_name}/data/"
        train_x = pd.read_csv(f"{data_path}train_x_1.0.0_binary.csv")
        train_t = pd.read_csv(f"{data_path}train_t_1.0.0_binary.csv").iloc[:, 0].values
        train_y = pd.read_csv(f"{data_path}train_y_1.0.0_binary.csv").iloc[:, 0].values.astype(float)
        test_x = pd.read_csv(f"{data_path}test_x_1.0.0_binary.csv")
        test_t = pd.read_csv(f"{data_path}test_t_1.0.0_binary.csv").iloc[:, 0].values
        test_y = pd.read_csv(f"{data_path}test_y_1.0.0_binary.csv").iloc[:, 0].values.astype(float)
    ...
    print(f"=== FAIRNESS ANALYSIS REGRESSION: {dataset_name.upper()} ===")
    ...
    baseline_results = doubly_robust_ate_cate_regression(full_y, use_test_portion=False)
    ...
    for a_f in alpha_range:
        ...
        try:
            ...
            train_baseline = pd.read_csv(f"{pred_folder}/Baseline_predictions_alpha_{a_f:.1f}/train_predictions_continuous.csv")['predictions_continuous'].values
            test_baseline = pd.read_csv(f"{pred_folder}/Baseline_predictions_alpha_{a_f:.1f}/test_predictions_continuous.csv")['predictions_continuous'].values
            combined_baseline = np.concatenate([train_baseline, test_baseline])
            
            baseline_results_alpha = doubly_robust_ate_cate_regression(combined_baseline, use_test_portion=False)
        ...
        # 2. Predictions Analysis (using combined train+test continuous predictions)
        try:
            pred_folder = f"./{dataset_name}_predictions"
            train_preds = pd.read_csv(f"{pred_folder}/CATE_predictions_alpha_{a_f:.1f}/train_predictions_continuous.csv")['predictions_continuous'].values
            test_preds = pd.read_csv(f"{pred_folder}/CATE_predictions_alpha_{a_f:.1f}/test_predictions_continuous.csv")['predictions_continuous'].values
            combined_preds = np.concatenate([train_preds, test_preds])
            
            # Use CATE predictions as outcomes on full dataset
            pred_results = doubly_robust_ate_cate_regression(combined_preds, use_test_portion=False)
        ...
        try:
            ...
            file_suffix = "binary_reg" if dataset_name in ["synthetic_class_data", "boston_housing_bin"] else "continuous_reg"
            ...
            pred_cf_results = doubly_robust_ate_cate_regression(combined_pred_cf, use_test_portion=False)
...
def plot_fairness_analysis_regression(results, alpha_range, dataset_name):
    """Plot ATE and CATE across alphas for all prediction types using regression approach"""
    ...
    ax1.set_title('Average Treatment Effect - Doubly Robust (Regression)')
    ax1.legend()
    ...
    ax2.set_title('Conditional Average Treatment Effect - Mean (Regression)')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plt.suptitle(f'Fairness Analysis (Regression): {dataset_name.replace("_", " ").title()}', fontsize=16, fontweight='bold')
    plt.tight_layout()
    
    # Save plot
    plt.savefig(f'{dataset_name}_fairness_analysis_regression.png', dpi=300, bbox_inches='tight')
    plt.close()

def main():
    """Run fairness analysis pipeline using regression approach for classification datasets"""
    print(f"=== FAIRNESS ANALYSIS REGRESSION PIPELINE: {DATASET.upper()} ===")
    
    # Run fairness analysis
    results, alpha_range = analyze_fairness_across_alphas_regression(DATASET)
    
    # Plot results
    plot_fairness_analysis_regression(results, alpha_range, DATASET)
    ...
    results_df.to_csv(f'{DATASET}_fairness_results_regression.csv', index=False)
    
    # Print summary
    print(f"\\\\n=== FAIRNESS SUMMARY REGRESSION: {DATASET.upper()} ===")
    ...
    print(f"\\\\nFairness analysis regression completed for {DATASET}!")
    print(f"Results saved: {DATASET}_fairness_analysis_regression.png, {DATASET}_fairness_results_regression.csv")
