...
def generate_predictions_counterfactuals_regression(dataset_name):
    """Generate prediction counterfactuals using regression CATE approach for classification datasets"""
    
    print(f"=== PREDICTION COUNTERFACTUALS REGRESSION: {dataset_name.upper()} ===")
    ...
    for alpha_val in alpha_range:
        ...
        if not os.path.exists(os.path.join(case_path, "test_predictions_continuous.csv")):
            print(f"Missing test_predictions_continuous.csv in {case_name}, skipping...")
            continue
        
        try:
            # Load continuous predictions from fair_model (regression approach)
            test_predictions = pd.read_csv(os.path.join(case_path, "test_predictions_continuous.csv"))
            train_predictions = pd.read_csv(os.path.join(case_path, "train_predictions_continuous.csv"))
            ...
            # Calculate correlation with test predictions (continuous values)
            ...
            version = "1.0.0"
            file_suffix_save = "binary_reg" if dataset_name in ["synthetic_class_data", "boston_housing_bin"] else "continuous_reg"
    ...
    if correlation_results:
        ...
        plt.title(f'{dataset_name.replace("_", " ").title()} - Prediction Counterfactuals Correlation (Regression)')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'{dataset_name}_predictions_counterfactuals_regression_correlation.png'), 
        ...
        results_df.to_csv(os.path.join(output_dir, f'{dataset_name}_predictions_counterfactuals_regression_results.csv'), index=False)
...
def main():
    """Generate prediction counterfactuals using regression CATE for classification datasets"""
    ...
    results = generate_predictions_counterfactuals_regression(DATASET)
