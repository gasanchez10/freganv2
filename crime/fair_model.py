...
def fair_metrics_calculator(train_x, train_y_factual, train_y_counterfactual, test_x, test_y, train_t, test_t, combination_type="average", dataset_name="", alpha_range=None):
    ...
    for a_f in alpha_range:
        # For baseline case (counterfactual = factual), always use factual
        if dataset_name == "Baseline":
            train_y_combined = train_y_factual.iloc[:, 0]
        else:
...
def load_cate_counterfactuals():
    ...
    try:
        ...
        # Extract counterfactuals based on treatment assignment (inverted for correct direction)
        train_cf_cate = []
        for i in range(len(train_potential)):
            if train_t.iloc[i, 0] == 0:  # Control group (high white pop), counterfactual is Y0 (keep same)
                train_cf_cate.append(train_potential.iloc[i, 0])
            else:  # Treatment group (low white pop), counterfactual is Y1 (keep same)
...
def run_fair_model_analysis():
    ...
    for combo_type in combination_types:
        ...
        if alphas is not None and all_results:
            ...
            plt.title(f'Fair Model Performance - Crime', fontsize=14)
            plt.legend()
