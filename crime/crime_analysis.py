...
print("First 5 records:", X.head())

# Prepare features and target
y = y.iloc[:, 0]  # Use first target column (ViolentCrimesPerPop)
...
# Use race percentage as sensitive attribute - find race-related columns
race_cols = [col for col in X.columns if 'race' in col.lower() or 'black' in col.lower() or 'white' in col.lower() or 'hisp' in col.lower() or 'asian' in col.lower() or 'PerCap' in col or 'Polic' in col]
print(f"\nRace-related columns found: {race_cols}")

# Use racePctWhite as sensitive attribute (percentage of white population)
if 'racePctWhite' in X.columns:
    sensitive_col = 'racePctWhite'
    print(f"Using {sensitive_col} as sensitive attribute")
    print(f"{sensitive_col} range: {X[sensitive_col].min():.4f} - {X[sensitive_col].max():.4f}")
    
    # Create binary treatment: low white population (<median) vs high
    median_white = X[sensitive_col].median()
    X['low_white_pop'] = (X[sensitive_col] < median_white).astype(int)
    print(f"Low white population (<{median_white:.3f}): {X['low_white_pop'].sum()} communities")
    print(f"High white population (≥{median_white:.3f}): {(1-X['low_white_pop']).sum()} communities")
    
    # Remove all other race variables to avoid information leakage
    other_race_cols = [col for col in race_cols if col != sensitive_col]
    X = X.drop(columns=other_race_cols)
    print(f"Removed {len(other_race_cols)} other race variables: {other_race_cols}")
...
plt.tight_layout()
plt.savefig('crime_results.png', dpi=300, bbox_inches='tight')

print(f"\nTop 5 important features:")
print(importance.head().to_string())
print("\nMain results plot saved as 'crime_results.png'")

# Doubly Robust ATE Estimation
def doubly_robust_ate(X, y, treatment_col='low_white_pop'):
    """
    Estimate Average Treatment Effect using Doubly Robust estimation.
    """
    from sklearn.ensemble import RandomForestClassifier, RandomForestRegressor
    
    # Treatment is high_black_pop column
...
print(f"Treatment: High Black Population")
...
print(f"\nInterpretation: High black population effect on crime rate is {dr_results['ate_doubly_robust']:.4f} points (on average)")

# Plot ATE analysis
plt.figure(figsize=(20, 5))

# Plot 1: Crime Rate Distribution by Treatment
plt.subplot(1, 4, 1)
T = X['low_white_pop'].astype(int)
plt.hist(y[T==0], alpha=0.7, label='Low Black Pop', bins=20, density=True)
plt.hist(y[T==1], alpha=0.7, label='High Black Pop', bins=20, density=True)
plt.xlabel('Crime Rate')
plt.ylabel('Density')
plt.title('Crime Rate Distribution by Black Population')
plt.legend()
plt.grid(True, alpha=0.3)
...
plt.hist(ps[T==0], alpha=0.5, label='Low Black Pop (Control)', bins=20, density=True)
plt.hist(ps[T==1], alpha=0.5, label='High Black Pop (Treated)', bins=20, density=True)
...
y_control = y[T==0]
y_treated = y[T==1]
plt.boxplot([y_control, y_treated], labels=['Low Black Pop', 'High Black Pop'])
...
if len(numeric_cols) > 1:
    key_var = numeric_cols[0] if numeric_cols[0] != 'high_black_pop' else numeric_cols[1]
    var_median = X[key_var].median()
    high_var = X[key_var] > var_median
    
    # Calculate doubly robust ATE for each subgroup
    ps = dr_results['propensity_scores']
    X_features = X.drop(columns=['low_white_pop'])
    mu0 = RandomForestRegressor(n_estimators=100, random_state=42).fit(X_features[T==0], y[T==0]).predict(X_features)
    mu1 = RandomForestRegressor(n_estimators=100, random_state=42).fit(X_features[T==1], y[T==1]).predict(X_features)
    
    ate_high = np.mean((T[high_var]*(y[high_var] - mu1[high_var])/ps[high_var] + mu1[high_var]) - ((1-T[high_var])*(y[high_var] - mu0[high_var])/(1-ps[high_var]) + mu0[high_var]))
    ate_low = np.mean((T[~high_var]*(y[~high_var] - mu1[~high_var])/ps[~high_var] + mu1[~high_var]) - ((1-T[~high_var])*(y[~high_var] - mu0[~high_var])/(1-ps[~high_var]) + mu0[~high_var]))
...
plt.tight_layout()
plt.savefig('crime_ate_analysis.png', dpi=300, bbox_inches='tight')
print("\nATE analysis plots saved as 'crime_ate_analysis.png'")

# Conditional Average Treatment Effect (CATE) Estimation
def doubly_robust_cate(X, y, treatment_col='low_white_pop'):
    """
...
print(f"Std CATE (DR): {np.std(cate_results['cate_doubly_robust']):.4f}")
...
for var in numeric_cols[:3]:  # Check first 3 numeric variables
    if var != 'high_black_pop' and var in X.columns:
        var_values = X[var]
...
plt.tight_layout()
plt.savefig('crime_cate_analysis.png', dpi=300, bbox_inches='tight')
print("CATE analysis plots saved as 'crime_cate_analysis.png'")
...
# Use low_white_pop as treatment variable
treatment_col = 'low_white_pop'
...
print(f"Treatment variable: {treatment_col} (high black population)")
print(f"Outcome variable: Crime rate (continuous)")
print(f"Features: {train_x.shape[1]} variables (excluding treatment)")

print("\n" + "="*50)
print("ANALYSIS SUMMARY")
print("="*50)
print(f"Dataset: Communities and Crime (n={len(X)})")
print(f"Treatment: High Black Population")
print(f"Outcome: Crime rate (continuous)")
...
print("\nFiles generated:")
print("- crime_results.png (main analysis)")
print("- crime_ate_analysis.png (ATE plots)")
print("- crime_cate_analysis.png (CATE plots)")
