# CATE Counterfactuals Analysis for Crime (Continuous Regression)
...
print("=== CATE COUNTERFACTUALS ANALYSIS FOR CRIME (CONTINUOUS) ===")

# Load data
print("Loading crime data...")
train_x = pd.read_csv(os.path.join(data_dir, f'train_x_{version}_continuous.csv'))
train_t = pd.read_csv(os.path.join(data_dir, f'train_t_{version}_continuous.csv')).values.flatten()
train_y = pd.read_csv(os.path.join(data_dir, f'train_y_{version}_continuous.csv')).values.flatten()

test_x = pd.read_csv(os.path.join(data_dir, f'test_x_{version}_continuous.csv'))
test_t = pd.read_csv(os.path.join(data_dir, f'test_t_{version}_continuous.csv')).values.flatten()
test_y = pd.read_csv(os.path.join(data_dir, f'test_y_{version}_continuous.csv')).values.flatten()
...
def doubly_robust_cate(X, T, Y):
    """Calculate CATE using doubly robust estimation for continuous outcomes."""
    ...
    mu0 = RandomForestRegressor(n_estimators=100, random_state=42).fit(X[T==0], Y[T==0]).predict(X)
    mu1 = RandomForestRegressor(n_estimators=100, random_state=42).fit(X[T==1], Y[T==1]).predict(X)
...
# Clip to valid range [0,1] for crime rates
counterfactuals_clipped = np.clip(counterfactuals, 0, 1)
...
# Split counterfactuals back to train/test
train_cf_cate = counterfactuals_clipped[:len(train_y)]
test_cf_cate = counterfactuals_clipped[len(train_y):]

for i in range(len(train_y)):
    if train_t[i] == 0:  # Control (high white pop)
...
for i in range(len(test_y)):
    if test_t[i] == 0:  # Control (high white pop)
...
for name, array in zip(data_names, data_arrays):
    ...
    save_path = os.path.join(results_dir, f"{name}_{version}_continuous_cate.csv")
...
pd.DataFrame({'counterfactuals': counterfactuals_clipped}).to_csv(
...
fig.suptitle('CATE Counterfactuals Analysis - Crime', fontsize=16, fontweight='bold')
...
correlation = np.corrcoef(full_y, counterfactuals_clipped)[0, 1]
ax2.scatter(full_y, counterfactuals_clipped, alpha=0.6, color='blue')
