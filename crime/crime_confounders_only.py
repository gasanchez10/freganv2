import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import os
from sklearn.ensemble import RandomForestRegressor
from sklearn.model_selection import train_test_split
from sklearn.metrics import mean_squared_error
import warnings
warnings.filterwarnings('ignore')
from ucimlrepo import fetch_ucirepo

# Fetch dataset
communities_and_crime = fetch_ucirepo(id=183)
X = communities_and_crime.data.features
y = communities_and_crime.data.targets

print("Original dataset shape:", X.shape)

# Prepare features and target
y = y.iloc[:, 0]  # Use first target column (ViolentCrimesPerPop)

# Handle missing values (represented as '?' in this dataset)
X = X.replace('?', np.nan)
X = X.apply(pd.to_numeric, errors='coerce')

# Create binary treatment: racePctWhite (low white population = treated)
if 'racePctWhite' in X.columns:
    median_white = X['racePctWhite'].median()
    X['low_white_pop'] = (X['racePctWhite'] < median_white).astype(int)
    print(f"Treatment created: low_white_pop (<{median_white:.3f})")

# Handle missing values
X = X.fillna(X.median())
y = y.fillna(y.median())

print("Available variables:", list(X.columns))

# SELECT ONLY CONFOUNDER VARIABLES for crime analysis
# Key confounders that affect both treatment (racial composition) and outcome (crime):
confounder_vars = [
    'low_white_pop',     # Treatment variable (keep)
    'medIncome',         # Economic - affects both racial composition and crime
    'pctWPubAsst',       # Public assistance - economic indicator
    'pctUnemployed',     # Unemployment - economic/social
    'PctPopUnderPov',    # Poverty rate - key confounder
    'PctLess9thGrade',   # Education - affects opportunities
    'PctNotHSGrad',      # Education completion
    'PctBSorMore',       # Higher education
    'PctIlleg',          # Family structure
    'PctKids2Par',       # Two-parent families
    'NumInShelters',     # Housing instability
    'PctHousLess3BR',    # Housing density
    'MedRent',           # Housing costs
    'PctHousOccup',      # Housing occupancy
    'PersPerOccupHous',  # Household density
    'PctPersDenseHous',  # Housing density
    'PctForeignBorn',    # Immigration status
    'PctSpeakEnglOnly',  # Language/integration
    'PctLargHouseFam',   # Family size
    'PctYoungKids2Par'   # Family structure with young children
]

# Filter to only available confounders
available_confounders = [var for var in confounder_vars if var in X.columns]
print(f"\nSelected confounders ({len(available_confounders)}): {available_confounders}")

# Create filtered dataset with only confounders
X_confounders = X[available_confounders].copy()

print(f"Filtered dataset shape: {X_confounders.shape}")
print(f"Reduction: {X.shape[1]} -> {X_confounders.shape[1]} variables")

# Train model on confounders only
X_train, X_test, y_train, y_test = train_test_split(X_confounders, y, test_size=0.2, random_state=42)
rf_model = RandomForestRegressor(n_estimators=100, random_state=42)
rf_model.fit(X_train, y_train)

# Predict and calculate MSE
y_pred = rf_model.predict(X_test)
mse = mean_squared_error(y_test, y_pred)
rmse = np.sqrt(mse)

print(f"\nModel Performance (Confounders Only):")
print(f"MSE: {mse:.4f}")
print(f"RMSE: {rmse:.4f}")

# Doubly Robust ATE with confounders only
def doubly_robust_ate(X, y, treatment_col='low_white_pop'):
    from sklearn.ensemble import RandomForestClassifier, RandomForestRegressor
    
    T = X[treatment_col].astype(int)
    X_features = X.drop(columns=[treatment_col])
    
    # Estimate propensity scores and outcome models
    ps = RandomForestClassifier(n_estimators=100, random_state=42).fit(X_features, T).predict_proba(X_features)[:, 1]
    mu0 = RandomForestRegressor(n_estimators=100, random_state=42).fit(X_features[T==0], y[T==0]).predict(X_features)
    mu1 = RandomForestRegressor(n_estimators=100, random_state=42).fit(X_features[T==1], y[T==1]).predict(X_features)
    
    # Doubly Robust ATE
    ate_dr = np.mean((T*(y - mu1)/ps + mu1) - ((1-T)*(y - mu0)/(1-ps) + mu0))
    ate_reg = np.mean(mu1 - mu0)
    
    return {
        'ate_doubly_robust': ate_dr,
        'ate_regression_only': ate_reg,
        'treated_fraction': np.mean(T)
    }

# Calculate ATE with confounders only
dr_results = doubly_robust_ate(X_confounders, y)

print(f"\nCausal Analysis (Confounders Only):")
print(f"ATE (Doubly Robust): {dr_results['ate_doubly_robust']:.4f}")
print(f"ATE (Regression): {dr_results['ate_regression_only']:.4f}")
print(f"Treated fraction: {dr_results['treated_fraction']:.3f}")

# Export filtered data for counterfactual analysis
print(f"\nExporting confounders-only data...")

data_dir = "data_confounders"
os.makedirs(data_dir, exist_ok=True)

# Use low_white_pop as treatment variable (already positive ATE)
treatment_col = 'low_white_pop'
train_t = X_train[treatment_col].values.reshape(-1, 1)
test_t = X_test[treatment_col].values.reshape(-1, 1)

# Features without treatment
train_x = X_train.drop(columns=[treatment_col]).values
test_x = X_test.drop(columns=[treatment_col]).values

# Outcomes
train_y = y_train.values.reshape(-1, 1)
test_y = y_test.values.reshape(-1, 1)

# Save to CSV files
version = "1.0.0"
data_files = {
    'train_x': train_x,
    'train_t': train_t,
    'train_y': train_y,
    'test_x': test_x,
    'test_t': test_t,
    'test_y': test_y
}

for name, data in data_files.items():
    df = pd.DataFrame(data)
    filepath = os.path.join(data_dir, f"{name}_{version}_continuous.csv")
    df.to_csv(filepath, index=False)
    print(f"Saved {name}: {data.shape} -> {filepath}")

print(f"\nConfounders-only data exported to '{data_dir}/' directory")
print(f"Treatment: low_white_pop (low white population areas)")
print(f"Features: {train_x.shape[1]} confounder variables")
print(f"Excluded: Non-confounder variables and race-related variables (except treatment)")