...
def ganite(train_x, train_t, train_y, test_x, test_y, test_t, parameters):
    ...
    T_all = np.reshape(np.concatenate((train_t, test_t), axis=0), [len(train_t)+len(test_t), 1])
    Y_all = np.reshape(np.concatenate((train_y, test_y), axis=0), [len(train_t)+len(test_t), 1])
    ...
    test_y_hat=sess.run(Y_tilde, feed_dict = {X: np.concatenate((train_x, test_x), axis=0) , T: np.reshape(np.concatenate((train_t, test_t), axis=0), [len(train_t)+len(test_t),1]), Y: np.reshape(np.concatenate((train_y,test_y), axis=0 ), [len(train_t)+len(test_t),1])})
